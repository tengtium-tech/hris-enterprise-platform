...
/// <summary>
/// Binds an <see cref="AttendancePolicy"/> to an organizational scope for an effective
/// period. A child entity of that policy, never its own root. Source:
/// docs/04-modules/attendance/domain/entities.md (PolicyAssignment).
///
/// Two assignments of different policies to the same scope target may not overlap
/// (AT-011); the parent's Assign/Unassign methods enforce that against the existing
/// set. A superseded assignment is end-dated and retained, never deleted (mirrors
/// Timekeeping's <c>ScheduleAssignment</c>).
/// </summary>
public sealed class PolicyAssignment : Entity<PolicyAssignmentId>
{
    public PolicyScopeLevel ScopeLevel { get; }
    ...
    internal PolicyAssignment(
        PolicyAssignmentId id, PolicyScopeLevel scopeLevel, string scopeTargetId, DateOnly effectiveFrom,
